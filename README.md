...
2. **Tag**: Tags image with Git SHA and `latest`
3. **Update Chart**: Updates the Helm chart's `values.yaml` with new image tag
4. **Commit**: Commits and pushes changes back to repository
...
This application is deployed via ArgoCD from the [infra-repo](https://github.com/CeClavijoBe/infra-repo).

### How it works

1. Developer pushes code to `main` branch
2. GitHub Actions builds and pushes Docker image
3. GitHub Actions updates `my-app-chart/values.yaml` with new image tag
4. ArgoCD (watching this repo) detects the change
5. ArgoCD automatically syncs the new version to Kubernetes
